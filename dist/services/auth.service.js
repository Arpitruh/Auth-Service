...
import { ErrorCode, conflict, unauthorized, } from '../lib/errors.js';
import { signAccessToken, signRefreshToken, verifyRefreshToken, verifyAccessToken, remainingTtlSeconds, } from '../lib/jwt.js';
...
export const authService = {
    async register(email, password, ctx = {}) {
    ...
    async login(email, password, ctx = {}) {
        ...
        if (failures >= ENV.LOCKOUT_THRESHOLD) {
            ...
            throw unauthorized('Invalid email or password.', ErrorCode.INVALID_CREDENTIALS);
        }
        const user = await prisma.user.findUnique({ where: { email } });
        // Same error for missing user and bad password to avoid user enumeration.
        const invalid = () => unauthorized('Invalid email or password.', ErrorCode.INVALID_CREDENTIALS);
        if (!user) {
    ...
    async refresh(presentedToken, ctx = {}) {
        ...
        try {
            ...
            throw unauthorized('Invalid or expired refresh token.', ErrorCode.TOKEN_EXPIRED);
        }
        ...
        if (!stored || stored.revokedAt !== null || stored.token !== presentedHash) {
            ...
            logger.error({ userId: payload.sub, jti: payload.jti, event: 'token_reuse' }, 'refresh token reuse detected; family revoked');
            throw unauthorized('Refresh token has been revoked.', ErrorCode.TOKEN_REUSE);
        }
        if (stored.expiresAt.getTime() < Date.now()) {
            ...
            throw unauthorized('Refresh token has expired.', ErrorCode.TOKEN_EXPIRED);
        }
        ...
        logger.info({ userId: user.id, event: 'refresh' }, 'refresh token rotated');
        return { user: publicUser, tokens };
    },
    /**
     ...
     */
    async logout(refreshTokenValue, accessTokenValue) {
...
async function recordFailedLogin(email) {
    ...
    logger.warn({ email, failures: count, event: 'login_failure' }, 'login failure recorded');
}
/** Revokes every non-revoked refresh token for a user (family revocation). */
async function revokeFamily(userId, reason) {
    const result = await prisma.refreshToken.updateMany({
        where: { userId, revokedAt: null },
        data: { revokedAt: new Date() },
    });
    logger.warn({ userId, revoked: result.count, reason, event: 'family_revoked' }, 'refresh token family revoked');
}
